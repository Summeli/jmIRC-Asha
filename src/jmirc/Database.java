/*
Copyright (C) 2005-2009  Juho Vähä-Herttua
...
public class Database {
	...
	String sharedProfileHost;
	int sharedProfilePort;
	...
	public void setSharedProfile(int index){
		if (index < 0) {
			sharedProfileName = "Default";
			sharedProfileHost = "irc.dal.net";
			sharedProfilePort = 6667;
		}
		else if (index < idxarray.length) {
			try {
				int rsidx = idxarray[index];
				...
				if (rsidx > 0) {
					...
					sharedProfileName =  dis.readUTF();
					//nick 
					dis.readUTF();
					//sharednick
					dis.readUTF();
					
					sharedProfileHost = dis.readUTF();
					sharedProfilePort = dis.readInt();				
				}
				rs.closeRecordStore();
			} catch (Exception e) {
				e.printStackTrace();
