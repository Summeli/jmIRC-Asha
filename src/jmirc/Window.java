...
public class Window extends Canvas implements CommandListener, GestureListener{
	...
	public Window(UIHandler uihandler, String name, int type, String hilight, boolean showheader, boolean timestamp, boolean usecol, boolean mirccol, int fontsize, int buflines) 
	{
		...
		if (!initialized) {
			...
			cmd_back = new Command("back", Command.BACK, 70);
		...
		gizCanvas = new GestureInteractiveZone( GestureInteractiveZone.GESTURE_ALL );
	...
	public void handleGestureCanvas(Object container, GestureInteractiveZone gestureZone, GestureEvent ge) {
		...
		switch (ge.getType()) {
			...
			case GestureInteractiveZone.GESTURE_FLICK: {
				float direction = ge.getFlickDirection();
				if( direction >= -0.79 && direction < 0.79){
					//RIGHT
					uihandler.displayNextWindow();
				} else if( direction >= 0.79 && direction < 2.36){
					//DOWN
					if (textarea.updatePosition(-1)) repaint();
				}else if(direction >= 2.36 || direction < -2.36){
					//LEFT
	...
	public void commandAction(Command c, Displayable s) {
		...
		else if (c == cmd_disconnect || c.getCommandType() == Command.BACK) {
